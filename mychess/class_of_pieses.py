...
class pawn(piece):
    ...
    def moving(self, m):
        ...
        temp2 = self.colmn.deepcope(self.colmn)
        if 1 <= m[1] <= 8 and (myfunction.isnt_balcked_pawn(self.side, self.row, self.colmn)\
            ...
            self.row = m[0]
            if self.side == "white":
                if myfunction.is_king_checked_there(self.side, chess_beord.kw.row, chess_beord.kw.colmn):
                    self.colmn = temp1
                    self.row = temp2
                    return False
            else:
                if myfunction.is_king_checked_there(self.side, chess_beord.kb.row, chess_beord.kb.colmn):
                    self.colmn = temp1
                    self.row = temp2
                    return True
