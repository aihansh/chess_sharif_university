...
def reeding_request(text, side):
    ...
    if temp == None:
        pattern = r"(\w{4,5})+casel"
        temp = re.search(pattern, text)
        t = myfunction.casel(side, temp.group(1))
        return t
        ...
        while True:
            ...
            if reeding_request(request_white, "white"):
                ...
                for row in board.board_bace:
                    print(' '.join(row))
                move_request_black = input(f"{secend.name} pleas write your move: ")
                if reeding_request(move_request_black, "black"):
                    imaplib.reload(board)
                    for row in board.board_bace:
                        print(' '.join(row))
                else:
                    while True:
                        ...
                        if reeding_request(move_request_black, "black"):
                            ...
                            for row in board.board_bace:
                                print(' '.join(row))
                        if move_request_black == "out" or request_white == "out":
